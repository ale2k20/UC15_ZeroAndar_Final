...
public class JdbcCorretorDAO implements CorretorDAO {
    ...
    public int inserir(PessoaCorretor corretor) throws SQLException {
        ...
        try {
            ...
            // Garantimos os valores obrigatorios do cadastro antes do INSERT.
            String creci = corretor.getCreci() == null ? null : corretor.getCreci().trim();
            java.util.Date dataAdmissao = corretor.getDataAdmissao() == null
                    ? new java.util.Date() : corretor.getDataAdmissao();
            ...
            try (PreparedStatement stmt = connection.prepareStatement(sqlCorretor, Statement.RETURN_GENERATED_KEYS)) {
                ...
                stmt.setString(2, creci);
                stmt.setDouble(3, corretor.getComissaoPercentual());
                stmt.setDate(4, new java.sql.Date(dataAdmissao.getTime()));
    ...
    /** Busca corretores diretamente no banco por nome, CPF, CRECI ou e-mail. */
    public List<PessoaCorretor> buscarPorTexto(String texto) throws SQLException {
        List<PessoaCorretor> lista = new ArrayList<>();
        String sql = "SELECT p.*, pc.* FROM Pessoa p INNER JOIN PessoaCorretor pc ON p.id_pessoa = pc.id_pessoa "
                + "WHERE p.ativo = true AND (p.nome LIKE ? OR p.cpf LIKE ? OR p.email LIKE ? OR pc.creci LIKE ?) ORDER BY p.nome";
        String termo = "%" + texto.trim() + "%";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            stmt.setString(1, termo); stmt.setString(2, termo); stmt.setString(3, termo); stmt.setString(4, termo);
            try (ResultSet rs = stmt.executeQuery()) { while (rs.next()) lista.add(extrairCorretor(rs)); }
        }
        return lista;
    }
    ...
    private PessoaCorretor extrairCorretor(ResultSet rs) throws SQLException {
        ...
        corretor.setTelefone(buscarTelefonePrincipal(rs.getInt("id_pessoa")));

...
import uc15.etapa9.zeroandar.repository.jdbc.JdbcClienteDAO;
...
public class ClienteService {
    ...
    public boolean adicionar(PessoaCliente cliente) {
        try {
            // Na inclusao seguimos a mesma regra usada pelos demais CRUDs:
            // os campos obrigatorios do formulario sao tratados pelo frontend
            // e o banco valida as restricoes de unicidade e integridade.
            // A validacao matematica de CPF nao deve bloquear um cadastro
            // quando a tela estiver trabalhando com dados de teste.
    ...
    public boolean atualizar(PessoaCliente cliente) {
        if (cliente == null || !validacao.validarNome(cliente.getNome())) {
            return false;
        }
        // O script inicial do projeto possui CPFs demonstrativos que nao
        // passam pela validacao matematica. Em uma alteracao, mantemos o
        // CPF existente e permitimos atualizar os demais dados do cliente.
        if (cliente.getEmail() != null && !cliente.getEmail().isEmpty()
                && !validacao.validarEmail(cliente.getEmail())) {
            return false;
        }
        try {
    ...
    private boolean validarDados(PessoaCliente cliente) {
        ...
        // No cadastro, aceitamos CPF informado com 11 dígitos.
        // A tela pode usar CPF de teste; a unicidade continua sendo
        // garantida pelo banco de dados.
        if (cliente.getCpf() == null
                || cliente.getCpf().replaceAll("[^0-9]", "").length() != 11) {
            return false;
        }
        if (cliente.getEmail() != null && !cliente.getEmail().isEmpty()) {

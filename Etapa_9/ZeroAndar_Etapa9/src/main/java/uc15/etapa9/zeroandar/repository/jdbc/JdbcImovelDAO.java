...
import uc15.etapa9.zeroandar.repository.ImovelDAO;
...
public class JdbcImovelDAO implements ImovelDAO {
    ...
    public List<Imovel> listarDestaques() throws SQLException {
        ...
        String sql = "SELECT * FROM Imovel WHERE status = 'disponivel' " +
                "ORDER BY destaque DESC, data_criacao DESC LIMIT 3";
    ...
    /** Busca imóveis diretamente no banco por código, descrição, tipo ou cidade. */
    public List<Imovel> buscarPorTexto(String texto) throws SQLException {
        List<Imovel> lista = new ArrayList<>();
        String sql = "SELECT i.* FROM Imovel i INNER JOIN Endereco e ON i.id_endereco = e.id_endereco "
                + "WHERE i.codigo_imovel LIKE ? OR i.descricao LIKE ? OR i.tipo LIKE ? "
                + "OR e.rua LIKE ? OR e.bairro LIKE ? OR e.cidade LIKE ? ORDER BY i.codigo_imovel";
        String termo = "%" + texto.trim() + "%";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            for (int n = 1; n <= 6; n++) stmt.setString(n, termo);
            try (ResultSet rs = stmt.executeQuery()) { while (rs.next()) lista.add(extrairImovel(rs)); }
        }
        return lista;
    }
    ...
    public boolean excluir(String codigoImovel) throws SQLException {
        ...
        try {
            // Primeiro descobrimos o ID e o endereço do imóvel.
            int idImovel = 0;
            int idEndereco = 0;
            String busca = "SELECT id_imovel, id_endereco FROM Imovel WHERE codigo_imovel = ?";
            try (PreparedStatement stmt = connection.prepareStatement(busca)) {
                stmt.setString(1, codigoImovel);
            ...
            // Algumas tabelas do script original não possuem ON DELETE CASCADE.
            // Removemos primeiro os registros dependentes do imóvel.
            String[] dependencias = {
                "DELETE FROM Interacao WHERE id_imovel = ?",
                "DELETE FROM Agenda WHERE id_imovel = ?",
                "DELETE FROM Negociacao WHERE id_imovel = ?",
                "DELETE FROM Avaliacao WHERE id_imovel = ?",
                "DELETE FROM Anexo WHERE id_imovel = ?",
                "DELETE FROM ImovelCaracteristica WHERE id_imovel = ?"
            };
            ...
            try (PreparedStatement stmt = connection.prepareStatement(sql)) {
                stmt.setInt(1, idImovel);
                removido = stmt.executeUpdate();
            }

            // O endereço pertence ao imóvel. Só removemos se não estiver sendo
            // usado por outra tabela/pessoa.
            if (removido > 0 && idEndereco > 0) {
                String verifica = "SELECT COUNT(*) FROM Imovel WHERE id_endereco = ?";
                try (PreparedStatement stmt = connection.prepareStatement(verifica)) {
                    stmt.setInt(1, idEndereco);
                    try (ResultSet rs = stmt.executeQuery()) {
                        if (rs.next() && rs.getInt(1) == 0) {
                            try (PreparedStatement del = connection.prepareStatement("DELETE FROM Endereco WHERE id_endereco = ?")) {
                                del.setInt(1, idEndereco);
                                del.executeUpdate();
                            }
                        }
                    }
